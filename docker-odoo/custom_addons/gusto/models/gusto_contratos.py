...
class GustoContratos(models.Model):
    ...
    def _calculate_days(self):
        for rec in self:
            ...
            # SACAR DE AQUI
            if rec.suma_diascomputable < 120:
                rec.fecha_objetivo = date.today()+timedelta(days=(((120-rec.suma_diascomputable)*100)/(rec.porcentaje_jornada)))
            else:
                #recorrer los contratos para calcular cuando cumple
                rec.fecha_objetivo = date.today()-timedelta(days=(((rec.suma_diascomputable-120)*100)/(rec.porcentaje_jornada)))
